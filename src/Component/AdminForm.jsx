...
const AdminForm = () => {
  ...
  const [editingFeadibilityIndex, setEditingFeadibilityIndex] =
    useState(Number);
          ...
          <div className="col-6">
            <input
              ...
              className={`form-control ${
                v.attribute_name === "" && validation ? "border-danger" : ""
              }`}
              onChange={(e) => handleCriteriaModalIp(e, v.attribute_id)}
              required
            />
          </div>
        </>
  ...
  const handleCriteriaModalIp = (event, criteriaId) => {
    var newCriteriaArray = criteriaDataDupli.map((v) => {
      return v.attribute_id === criteriaId
        ? { ...v, attribute_name: event.target.value }
        : v;
    });
  ...
  const handleSaveChanges = (modalName) => {
    if (modalName === "feasibility") {
      if (question !== "" && boundaryValue !== "") {
        ...
        const checkbountryValueisFinite = checkingIfAnythingIsEmpty.filter(
          (v) => {
        ...
        if (questionType === "numeric") {
          if (
            checkbountryValueisFinite.length ===
            checkingIfAnythingIsEmpty.length
          ) {
    ...
    <div className="admin-page-content-height py-4 placeholder-glow">
      <div className="card h-100 overflowY overflowX py-3">
        <div className="w-100 p-3">
          <button
            className="btn btn-outline-secondary d-flex align-items-center gap-2 ms-1 py-1 px-2"
            onClick={() => {
              pageRender("/");
            }}
          >
                ...
                <div className="col-6 mb-3">
                  ...
                  >
                    ...
                    {clientDetails.map((v, i) => {
                      return v.name === "Toyota" ? null : (
                        <option value={[v.id, v.services[0].id]} key={i}>
                          {v.name}
                        </option>
                      );
                    })}
                ...
                <div className="col-6 mb-3">
                  {glowOne ? (
                    <>
                      <label
                        htmlFor="clientName"
                        className="form-label placeholder py-2 pb-3 w-50 rounded"
                        aria-label="Default select example"
                      ></label>
                      <select
                        className="form-select placeholder py-4=3 w-100 rounded"
                        id="clientName"
                      ></select>
                    </>
                  ) : (
              ...
              <div className="card">
                <div className="card-body">
                  {feasabilityPlaceholder.map((v, i) => {
                    return (
                      <div
                        className="col-12 d-flex flex-wrap gy-3 pt-4 border-bottom p-2 my-2 rounded"
                        key={i}
                      >
                        <div className="col-12 position-relative">
                          <p className="placeholder py-2 pb-3 w-25 rounded"></p>
                          <p className="placeholder py-4 w-100 rounded">
                            {v.question}
                          </p>
                          <div className="edit-btn">
                            <button
                              type="button"
                              className="btn btn-primary px-5 placeholder py-1"
                            ></button>
                          </div>
                        </div>
                              ...
                              return (
                                <div className="col-6">
                                  <div className="card placeholder w-100 py-2">
                                    <div className="card-body py-2 pb-3"></div>
                                  </div>
                                </div>
                              );
                            })}
                      ...
                      return (
                        <div className="col" key={i}>
                          <div className="card py-3 placeholder w-100">
                            <div className="card-body py-1"></div>
                          </div>
                        </div>
                      );
                    })}
                      ...
                      <div className="col-12 position-relative">
                        ...
                        <div className="edit-btn">
                          <button
                            ...
                            data-bs-target="#feasibilityModal"
                          >
                            ...
                            ? v.bountary_value.map((v, i) => {
                                return (
                                  <div className="col-12 col-md-6" key={i}>
                                    <div className="card">
                                      <div className="card-body py-1">{v}</div>
                                    </div>
                                  </div>
                                );
                              })
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">
                Question {editingFeadibilityIndex + 1}
              </h5>
              <button
                type="button"
                className="btn-close"
                ...
                <div className="col-12">
                  <div className="mb-3">
                    ...
                    <input
                      type="text"
                      className={`form-control ${
                        question === "" && validation ? "border-danger" : ""
                      }`}
                  ...
                  <div className="mb-3">
                    <label htmlFor="exampleInputBountry" className="form-label">
                      Value
                    </label>
                    {originalBoundaryType === "value" ||
                    originalBoundaryType === "in" ? (
                      <>
                        <textarea
                          rows={4}
                          className={`form-control ${
                            boundaryValue === "" && validation
                              ? "border-danger"
                              : ""
                          }`}
                          id="exampleInputBountry"
                          value={boundaryValue}
                    ...
                    <div className="mb-3">
                      ...
                      <select
                        className={`form-select w-50 ${
                          bountryType === "" && validation
                            ? "border-danger"
                            : ""
                        }`}
                        value={bountryType}
                        onChange={(e) => setBountryType(e.target.value)}
                      >
                        {dynamicBountryType()}
                      </select>
                    </div>
                  ...
                  <div className="mb-3">
                    <label htmlFor="exampleInputTooltip" className="form-label">
                      Tool tip
                    </label>
                    <input
                      type="text"
                      className={`form-control ${
                        toolTip === "" && validation ? "border-danger" : ""
                      }`}
                      id="exampleInputTooltip"
                      value={toolTip}
                      onChange={(e) => serToolTip(e.target.value)}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="modal-footer">
              ...
              <button
                type="button"
                className="btn btn-primary"
                onClick={() => handleSaveChanges("feasibility")}
              >
                Save changes
              </button>
            </div>
      ...
      >
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="row gy-3">{handleCriteriaEdit()}</div>
            </div>
            <div className="modal-footer">
              <button
                type="button"
              ...
              <button
                type="button"
                className="btn btn-primary"
                onClick={() => handleSaveChanges("criteria")}
              >
                Save changes
              </button>
            </div>
